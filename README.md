...
Kimi Code, Pi, Oh My Pi, ZCode, Antigravity CLI, DeepSeek Harness, and Grok Build are passive, precise sources: they keep raw model names, use source-scoped cursors for incremental/idempotent replay, and never persist transcript text. Pi and Oh My Pi share one parse implementation and keep separate cursors. Pi support is verified with local Oh My Pi samples plus sanitized Pi-compatible fixtures; Pi-only local evidence is still limited. Grok Build maps each `turn_completed` `params.update.usage` record to one event (input, cache read, cache creation, output, diagnostic reasoning, authoritative total). Sessions without usage keep the older total-only fallback. A sidecar change replays the whole session. Cost stays `unpriced` because the catalog has no grok row, and `costUsdTicks` is not used. `source-status` and `dash` also show monitor-only platform candidates such as Reasonix, Gemini CLI, Cursor, Copilot, Zed, Kiro, Goose, Kimi shell/Qwen, Roo/Kilo/Cline, Codebuff, Crush, Warp/Oz, Amp, Hermes, and Trae. Monitor-only means llmusage can probe candidate local roots and explain why parsing is blocked; it does not write zero usage rows or untrusted token rows.
