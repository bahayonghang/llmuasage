...
human 输出默认聚合所有匹配来源为一张 ccusage 风格 daily 表，标题为 `LLM Usage Report - Daily`。默认列为 `Date / Models / Input / Output / Cache Create / Cache Read / Total Tokens / Cost (USD)`；宽屏下 token 使用完整逗号分隔数字，模型以多行 `- model` 列表展示，`NO_COLOR=1` 会禁用 ANSI 样式。Source 维度通过 `--source` 过滤和 `--breakdown` 按来源/模型明细保留。`--json` 仍是稳定的聚合 snake_case payload，并包含 `cache_creation_tokens`。
...
在 `127.0.0.1` 启动本地分析页和 JSON API，并默认用系统浏览器打开分析页。Dashboard 初始加载使用 `/api/dashboard` 单连接快照，让 overview、趋势、模型/来源/项目/成本排行、health 和 diagnostics 共用同一个 `source` / `model` / `since` / `until` / `window` 筛选。live 模式还提供 JSON 导出、可选 30s/60s 自动刷新，以及 sync job 启动/取消/进度控件。
...
导出的静态目录复用同一套 Dashboard shell，并把导出时的筛选写入 `snapshot.json`；实时 sync 与自动刷新控件会被禁用并显示说明。
