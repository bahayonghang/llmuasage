...
六张摘要卡会随当前筛选显示会话数、请求数、Token 用量、估算成本、活跃天数和缓存读取占比；高亮的 Token 用量卡还会标出用量最高的来源。每日活跃度可以切换 Token 用量 / 请求数强度，支持键盘焦点和提示信息；点击日期会把全局筛选下钻到当天，再次点击则恢复之前的范围。
...
每周活跃时段会按浏览器 IANA 时区把 30 分钟桶折叠成周一优先的 `7 x 24` 网格；宽屏下它与每日活跃度并排，中窄屏则自动纵向排列。高用量会话支持由服务端按 Token 用量、活跃时长和估算成本排序；点击会话会跳转到事件日志，并由服务端过滤该会话。展开事件行时才按需读取保留的原始 JSON。事件日志仅在实时模式可用，沿用每页 50 条的游标分页。
