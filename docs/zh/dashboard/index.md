...
3. 查看每日活跃度、每周活跃时段和范围感知的 Token 用量构成，再用短时趋势观察近 24 小时的发生时间。
4. 对比会话消耗排行与项目、模型、来源和成本排行。
5. 查看行为面板：活动类型、工具使用、优化建议和模型对比。
6. 用“用量分析”回答临时的本地多维分析问题。
7. 在事件日志中分页查看事件细节；数据过旧时使用同步、CSV 导出或诊断信息。

屏幕宽度不超过 `720px` 时，数据状态卡会收敛为首屏内的紧凑折叠摘要。其标题只跟随当前 worker lock 与最新用量导入结果；已恢复的 `serve` 中断和其他历史命令失败仍保留在运行诊断中，但不会再把当前数据状态变成警告。宽屏仍显示完整状态卡；看板不再展示集成安装状态。
...
Token 用量构成以持久化的 `total_tokens` 为权威总量，分开展示输入、缓存读取、缓存写入和输出 Token，并把尚未由这些通道解释的差额显示为“其他 / 未细分”，不会错误并入输出。近 1 天或单日范围会把接口返回的本地日片段聚合成一条构成条，同时显示精确值和占比；多日范围则用相同的五类通道绘制每日堆叠柱，键盘与鼠标用户都能读取每日总量和估算成本。如果已知通道之和异常大于权威总量，面板会明确报告数据质量问题，而不是绘制负数或静默改写总量。实时模式下，这些面板与活动类型、工具使用、优化建议、用量分析和模型对比一样进入“最新请求优先”的次级加载生命周期，因此旧响应不能覆盖新筛选。
...
每周活跃时段会按浏览器 IANA 时区把 30 分钟桶折叠成周一优先的 `7 x 24` 网格；宽屏下它与每日活跃度并排，近 1 天则独占整行，中窄屏自动纵向排列。会话消耗排行使用横向条形图，并由服务端按 Token 用量、活跃时长和估算成本排序。每行只显示项目、已注册 Agent 名称和当前筛选范围内的首末事件时间，技术 session ID 不进入可视或无障碍标签；点击后仍用 canonical session 精确过滤事件日志。展开事件行时才按需读取保留的原始 JSON。事件日志仅在实时模式可用，沿用每页 50 条的游标分页。
...
实时看板会把当前已加载的摘要、每日趋势、项目、模型、来源与会话
导出为带 UTF-8 BOM 的 CSV；看板隐藏技术 session ID 不会改变 CSV 中既有的机器可读 session 字段。不可信标签会先做公式注入防护，再按标准 CSV 规则转义。
离线 HTML bundle 使用：
...
静态包的 `snapshot.json` 会包含摘要卡、每日活跃度、每周活跃时段、会话消耗排行、每日 Token 序列、默认用量分析数据和对应渲染资产。旧快照缺少新增会话时间字段时会使用 Agent、事件数和活跃时长回退，不会显示技术 ID。离线快照会禁用实时用量分析控件，并把事件日志标记为仅实时看板可用。
