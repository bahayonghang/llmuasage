...
trait 签名为同步 `&self`。Codex/Claude/Opencode 各用 ZST + impl 委托给原模块级 `pub fn probe / install / uninstall`（保留 pub fn 是因为 `tests/cli/local_flow.rs` 已直接调；trait 这层是新增的入口）。
...
- `tests/cli/local_flow.rs` 现在依赖 `integrations::HookTarget::current(...).shell_command(...)` 入口；后续若要 mock 平台，可以扩 `HookKind` 不破坏调用方。

## 验证

- 阶段 3 完成时：`rtk cargo build` / `cargo fmt --check` / `clippy -D warnings` / `cargo test --test-threads=1` 全绿（35/35 测试）。
- 阶段 4 完成时：同上（35/35 测试）；`tests/cli/local_flow.rs` 的 Windows 空格路径覆盖走新 `HookTarget` 路径无回归。
