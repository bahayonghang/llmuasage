...
This page is aligned with `cargo run -- --help`, `cargo run -- serve --help`, and `cargo run -- export html --help` for version `1.1.0`. Top-level help is rendered as a compact table; command-specific help still uses the clap output.
