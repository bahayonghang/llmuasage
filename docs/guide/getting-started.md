...
- `llmusage` without a subcommand prints the last 7 calendar days from the local DB, including today, split into colored source tables for Codex, Claude, and any other sources with data. Tables are separated with `---`; human daily tables use compact `K` / `M` / `B` token units and a `Notes` column for unpriced / not-reported metadata. `NO_COLOR=1` disables ANSI styling, and `llmusage daily --json` keeps the stable aggregate JSON shape. Use `llmusage daily --all` for full history, or `llmusage monthly`, `llmusage session`, and `llmusage blocks` for other report views.
