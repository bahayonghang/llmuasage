...
impl Dashboard {
    ...
    pub(super) fn tool_attribution_rows(
        ...
        self.tool_attribution_rows_sql(filter)
    }

    fn tool_attribution_rows_sql(&self, filter: &QueryFilter) -> Result<Vec<AttributedToolRow>> {
    ...
    pub(super) fn legacy_tool_attribution_rows(
        ...
        let event_rows =
            event_stmt.query_map(params_from_iter(event_filter.params().iter()), |row| {
        ...
        if has_event_filter {
            ...
            for event_keys in missing_tool_event_keys.chunks(500) {
                ...
                let mut stmt = self.conn.prepare(&sql)?;
                let rows = stmt.query_map(params_from_iter(event_keys.iter()), |row| {
                    Ok(ToolEventAttribution {
                        event_key: row.get(0)?,
                        event_at: row.get(1)?,
                        session_id: row.get(2)?,
