...
async fn run_daily(app: &AppContext, source: SourceKind, args: DailyArgs) -> Result<()> {
    ...
    let report = reports::load_unified_report(conn, &filter, PeriodKind::Daily)?;
...
async fn run_period(
    ...
    if !unified.sections.is_empty() {
        let reports = focused_sections(conn, &filter, source, kind, &unified, false)?;
    ...
    let report = reports::load_unified_report(conn, &filter, kind)?;
    print_focused_report(
        conn,
        &filter,
        &unified_report::focused_report(&report, source),
...
fn focused_sections(
    ...
    unified_report::load_sections(conn, filter, command_kind, &unified.sections, daily_all).map(
        |reports| {
            reports
                .iter()
...
fn print_focused_report(
    ...
    if json {
        ...
        unified_report::attach_hosts_json(conn, filter, report.kind, &mut value, no_cost)?;
        ...
        unified_report::print_host_section(conn, filter, report.kind, compact, no_cost)?;
    }
    Ok(())
}
...
fn print_focused_sections(
    ...
    if json {
        ...
        unified_report::attach_hosts_ordered(conn, filter, command_kind, &mut payload, no_cost)?;
        ...
        unified_report::print_host_section(conn, filter, command_kind, compact, no_cost)?;
    }
    Ok(())
}
