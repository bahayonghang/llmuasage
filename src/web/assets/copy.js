...
const UI_COPY_ZH = Object.freeze({
  ...
  readyWidgets: Object.freeze({
    ...
    trendsDaily: Object.freeze({
      title: 'Token 用量构成', sub: '单日查看范围构成，多日比较每天的 Token 组成。',
      input: '输入 Token', cacheRead: '缓存读取 Token', cacheCreation: '缓存写入 Token', output: '输出 Token', other: '其他 / 未细分', total: '权威总量', cost: '估算成本',
      empty: '当前筛选范围暂无 Token 用量。', loading: '正在加载 Token 用量构成…', degraded: 'Token 用量构成暂时不可用。',
      oneDayRange: '近 24 小时构成', sameDayRange: '所选日期构成', unknownDate: '未知日期',
      invalidData: 'Token 构成包含无效值，暂时无法可靠绘制。',
      inconsistent: '{date} 的已知通道合计 {known} 大于权威总量 {total}，请检查数据质量。',
    }),
  }),
  sessionAnalytics: Object.freeze({
    topSessions: Object.freeze({ title: '会话消耗排行', sub: '按当前指标比较会话；选择一项查看事件日志。', loading: '正在加载会话排行…', empty: '当前筛选范围暂无会话。', unknownAgent: '未知 Agent', sessionFallback: '{agent} 会话', snapshotFallback: '{agent} · {events} 个事件 · 活跃 {minutes} 分钟', openLogs: '{label}；{metric}；查看事件日志', sortLoading: '正在按“{metric}”刷新…', sortError: '未能刷新会话排序，已保留上一次结果。', sortAria: '会话排序指标', sort: Object.freeze({ tokens: 'Token 用量', duration: '活跃时长', cost: '估算成本' }) }),
...
const UI_COPY_EN = Object.freeze({
  ...
  readyWidgets: Object.freeze({
    ...
    trendsDaily: Object.freeze({
      title: 'Token usage mix', sub: 'See the range mix for one day or compare each day across longer ranges.',
      input: 'Input tokens', cacheRead: 'Cache-read tokens', cacheCreation: 'Cache-creation tokens', output: 'Output tokens', other: 'Other / unclassified', total: 'Authoritative total', cost: 'Estimated cost',
      empty: 'No token usage in this filter range.', loading: 'Loading token usage mix…', degraded: 'Token usage mix is temporarily unavailable.',
      oneDayRange: 'Last 24 hours mix', sameDayRange: 'Selected date mix', unknownDate: 'Unknown date',
      invalidData: 'The token mix contains invalid values and cannot be drawn reliably.',
      inconsistent: 'Known channels for {date} total {known}, above the authoritative total of {total}. Check data quality.',
    }),
  }),
  sessionAnalytics: Object.freeze({
    topSessions: Object.freeze({ title: 'Session consumption ranking', sub: 'Compare sessions by the selected metric; choose one to view its event logs.', loading: 'Loading session ranking…', empty: 'No sessions in this filter range.', unknownAgent: 'Unknown agent', sessionFallback: '{agent} session', snapshotFallback: '{agent} · {events} events · {minutes} active minutes', openLogs: '{label}; {metric}; view event logs', sortLoading: 'Refreshing by {metric}…', sortError: 'Could not refresh the session ranking. The previous result is still shown.', sortAria: 'Session ranking metric', sort: Object.freeze({ tokens: 'Token usage', duration: 'Active duration', cost: 'Estimated cost' }) }),
...
const SHELL_COPY_ZH = Object.freeze({
  ...
  'shell.status.title': '运行状态',
  'shell.status.sub': '当前同步健康状态以最近一次同步为准；诊断线索与最近失败仅供排查参考。',
...
const SHELL_COPY_EN = Object.freeze({
  ...
  'shell.status.title': 'Runtime status',
  'shell.status.sub': 'Sync health reflects the latest sync run; diagnostic signals and recent failures are for troubleshooting only.',
