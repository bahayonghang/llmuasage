...
function bindTooltip(container) {
  ...
  container.querySelectorAll('rect[data-tooltip]').forEach((cell) => {
...
export function renderCalendarHeatmap(context, state = {}) {
  ...
  const cells = rows.map((row, index) => {
    ...
    previousMonth = month;
    const tooltip = `${date} · ${formatNumber(row.event_count || 0)} ${copy.eventCount} · ${formatNumber(row.total_tokens || 0)} ${copy.tokenCount}`;
  ...
  container.querySelectorAll('rect[data-date]').forEach((cell) => {
