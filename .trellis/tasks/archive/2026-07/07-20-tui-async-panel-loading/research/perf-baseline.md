...
- 此处全史初测中 Behavior 达到 A3 的 >=30% 门槛，Stats 只达到 7.3%，当时 A3 未完成。
- 未降低门槛，也未把 UI 非阻塞等同于查询加速。后续由 `07-20-tui-time-window-bounding` 接入 30d 本地日历范围，并把全源 `context_pressure` 拆为逐 source 等值索引范围查询。

## 时间窗接入后的最终复测

协议、数据库与命令不变；窗口为 30d（`2026-06-22..2026-07-21`）。
...
Stats 与 Behavior 均达到 A3 的 >=30% 门槛。串行与并行路径使用相同窗口语义；单元测试验证 Stats/Behavior 载荷等值及逐 source context-pressure 加权合并等值。
