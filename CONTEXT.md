...
枚举 `SourceKind`（`src/domain/models.rs`），llmusage 当前支持的本地真源：
...
30 分钟 UTC 窗口。表 `usage_bucket_30m` 是所有 dashboard / report 查询的聚合源。每条 [`UsageEvent`](src/domain/models.rs) 落库时按 `(source, model, hour_start, project_hash)` 升级到对应 bucket（结构体 `BucketKey` / `BucketRollup` 在 `src/store/mod.rs` 内部）。
...
- `events`：本 shard 待写入的标准化 [`UsageEvent`](src/domain/models.rs)。Writer 按 `EVENT_WRITE_BATCH_SIZE` 内部切 chunk。
...
`src/registry.rs::registered_parsers()` / `registered_integrations()`。

工厂返回 `Vec<Box<dyn SourceParser>>` / `Vec<Box<dyn Integration>>`，是"系统支持哪些源"的唯一真源。`commands::sync::run_once` 与 `integrations::{probe_all, install_all, uninstall_all}` 都对 registry 做遍历。`crate::sources` 仅作为兼容 shim re-export registry，不再是实现文件。
...
它不是成本主事实表；成本/用量主路径仍是 [`UsageEvent`](src/domain/models.rs) 与 [`Bucket`](#6-bucket)。`UsageTurn` 只为 Activity / Optimize / Compare 提供 normalized 行为维度：source、session_id、source_path_hash、project_hash、primary_model、started_at、category、has_edits、retries、one_shot、call_count 与 token 汇总。
